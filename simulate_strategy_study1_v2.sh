...
#SBATCH --job-name="strategy analysis study 1 v2"

#SBATCH -o serial-R.out%j # capture jobid in output file name

# run simulation study using AWK script
awk -f simulate_scenarios.awk -v MODEL="limits to growth v2.stmx" strategy_study1.csv
...
cp study_results.csv strategy_study1_results.csv
